            ...
            <div>
              <div class="hs-dropdown relative inline-flex">
                ...
                <div class="overflow-y-scroll hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-[15rem] bg-white shadow-md rounded-lg p-2 mt-2 dark:bg-gray-800 dark:border dark:border-gray-700 dark:divide-gray-700 after:h-4 after:absolute after:-bottom-4 after:start-0 after:w-full before:h-4 before:absolute before:-top-4 before:start-0 before:w-full" aria-labelledby="hs-dropdown-default">
                  ...
                  <a href="{{ route('table_usulan_kegiatan', ['tahun' => $item->tahun]) }}" class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:bg-gray-700">
                    {{ $item->tahun }}
                  </a>
                  @endforeach
            ...
            <div>
              <div class="inline-flex gap-x-2">
                ...
                <form method="POST" action="{{ route('store_tahun') }}">
                  @csrf
                  <div id="hs-cookies" class="hs-overlay hidden w-full h-full fixed top-0 start-0 z-[60] overflow-x-hidden overflow-y-auto">
                      ...
                      <div class="relative flex flex-col bg-white shadow-lg rounded-xl dark:bg-gray-800">
                        ...
                        <div class="sm:p-14 overflow-y-auto">
                          ...
                          <div class="relative">
                            ...
                            @endif
                            <input name="tahun" id="hs-floating-input-email" class="peer p-4 block w-full border border-gray-200 rounded-lg text-sm placeholder:text-transparent focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600
                            focus:pt-8
                            focus:pb-2
                            [&:not(:placeholder-shown)]:pt-8
                            [&:not(:placeholder-shown)]:pb-2
                            autofill:pt-8
                            autofill:pb-2" placeholder="tahun anggaran">
                ...
                @if ($currentUsulan)
                  @if(count($currentUsulan->usulan_komponen_program) < 1)
                    <form method="POST" action="{{ route('store_table_usulan', ['name' => $currentUsulan->tahun]) }}">
                      @csrf
                ...
                @if($currentUsulan->usulan_komponen_program)
                  @foreach ($currentUsulan->usulan_komponen_program as $index => $item)
                    <tr class="bg-white hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-800">
                      <form method="POST" action="{{ route('update_usulan', ['id' => $item->id]) }}">
                        @csrf
                        @method('PUT')
                              ...
                              <div class="px-6 py-2">
                                <div class="block">
                                  <div class="hs-dropdown relative inline-flex [--placement:top-left]">
                                    <button id="hs-dropup" type="button" class="hs-dropdown-toggle py-3 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                                      {{ $item->komponen_program->kode ?? 'Pilih' }}
                                      <svg class="hs-dropdown-open:rotate-180 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m18 15-6-6-6 6"/></svg>
                                    </button>
                                    ...
                                    <div class="hs-dropdown-menu w-72 transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden z-10 bg-white shadow-md rounded-lg p-2 dark:bg-gray-800 dark:border dark:border-gray-700 dark:divide-gray-700" aria-labelledby="hs-dropup">
                                      @foreach ($komponen_program as $komponen_programs)
                                        <form method="POST" action="{{ route('update_usulan', ['id' => $item->id]) }}">
                                          ...
                                          @method('PUT')
                                            <button
                                            {{ $komponen_programs->id == $item->komponen_program_id ?? '' }} type="submit"
                                            name="komponen_program_id" value="{{ $komponen_programs->id }}" class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm font-normal text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:bg-gray-700">
                          ...
                          <a class="block relative z-10" href="#">
                            <div class="px-6 py-2">
                              @if ($item->komponen_program)
                                <p class="text-sm">{{ $item->komponen_program->uraian }}</p>
                              @else
                                -
                              @endif
                            </div>
                                  ...
                                  <div class="hs-dropdown-menu w-72 transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden z-10 bg-white shadow-md rounded-lg p-2 dark:bg-gray-800 dark:border dark:border-gray-700 dark:divide-gray-700" aria-labelledby="hs-dropup">
                                    @foreach ($satuan as $satuans)
                                      <form method="POST" action="{{ route('update_usulan', ['id' => $item->id]) }}">
                                        ...
                                        @method('PUT')
                                          <button
                                          {{ $satuans->id == $item->satuan_id ?? '' }} type="submit"
                                          name="satuan_id" value="{{ $satuans->id }}" class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm font-normal text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:bg-gray-700">
                                          {{ $satuans->satuan }}
                                          {{-- --
                                          {{ $komponen_programs->uraian }} --}}
                                          </button>
                              ...
                              <span class="inline-flex items-center gap-1.5 py-1 px-2 rounded-lg text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-200">
                                <form method="POST" action="{{ route('update_usulan', ['id' => $item->id]) }}">
                                  ...
                                  <div class="relative">
                                    ...
                                    [&:not(:placeholder-shown)]:pb-2
                                    autofill:pt-8
                                    autofill:pb-2" placeholder="you@email.com" value="{{ $item->harga_satuan }}">
                                    <label for="hs-floating-input-email" class="absolute top-0 start-0 p-4 h-full text-sm truncate pointer-events-none transition ease-in-out duration-100 border border-transparent dark:text-white peer-disabled:opacity-50 peer-disabled:pointer-events-none
                                      peer-focus:text-xs
                                      peer-focus:-translate-y-1.5
                                      peer-focus:text-gray-500
                        ...
                        <td class="h-px w-px whitespace-nowrap">
                          <div class="flex justify-end px-6 py-2">
                            <div class="flex hs-dropdown relative inline-block [--placement:bottom-right]">
                              <div>
                                    ...
                                    @foreach ($kategori as $index => $perulangan)
                                        @if ($index == count($currentUsulan->usulan_komponen_program))
                                            <form method="POST"
                                                ...
                                                @csrf
                                                @if($perulangan->kategori != 'Judul Kegiatan')
                                                <button type="submit"
                                                    class="py-2 px-3 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M2.63452 7.50001L13.6345 7.5M8.13452 13V2" stroke="currentColor" stroke-width="2" stroke-linecap="round" />
                                                    </svg>
                                                    @if($perulangan->kategori == 'Kegiatan' || $perulangan->kategori == 'KRO' || $perulangan->kategori == 'RO' || $perulangan->kategori == 'Komponen' || $perulangan->kategori == 'Judul Kegiatan')
                                                        {{ $perulangan->kategori }}
                                                    @endif
                                                </button>
                                                @endif
