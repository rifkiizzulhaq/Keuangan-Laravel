...
<div class="border bg-white rounded-xl shadow p-4 sm:p-7 dark:bg-slate-900">
  <form method="POST" action="{{ route('update_unit', [ 'id' => $user->id ]) }}">
    ...
    <div class="grid sm:grid-cols-12 gap-2 sm:gap-4 py-8 first:pt-0 last:pb-0 border-t first:border-transparent border-gray-200 dark:border-gray-700">
      ...
      <div class="sm:col-span-9">
        <input id="af-submit-application-email" value="{{ $user->unit->bidang }}" name="bidang" required="" placeholder="masukan nip" class="py-2 px-3 pr-11 block w-full border border-gray-200 shadow-sm text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400">
      </div>
