          ...
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400">Bidang</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              @foreach ($users as $item)
                <tr>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200 dark:bg-gray-500">{{ $item->pemimpin->bidang ?? '' }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200 dark:bg-gray-500">{{ $item->pemimpin->nip ?? ''  }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200 dark:bg-gray-500">{{ $item->pemimpin->nidn ?? ''  }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium dark:bg-gray-500 flex justify-end">
                    <a href="{{ route('edit_pemimpin', ['id' => $item->id]) }}" class="mx-2 py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold text-yellow-500 hover:bg-yellow-100 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800">
                      UPDATE
                    </a>
                    <form action="{{ route('destroy_pemimpin', ['id' => $item->id]) }}" method="POST">
                      @csrf
                      @method('DELETE')
